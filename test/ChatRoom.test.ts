...
describe('ChatRoom', () => {
    const senderPrivateKey =
        '0x435fe42bde8b4040646e37040a4f945a3233d7ede63d472d3ca46d3020ffb7a3'
    const receiverPrivateKey =
        '0xc0f76a931c4b42fa44ec3af1d2410b032fca23658c44b976299175ac2be15d96'
    let streamId: string
    ...
    it('should throw when attempting to create a locally-existent roomId', async () => {
        ...
        try {
            ...
            expect(e.message).toBe(
                `Room [${timestamp}](0x9374effd895610e89adbc17b632423ec254f351a/streamr-chat/room/${timestamp}) already exists`
            )
        }
    })
    ...
    it('should exercise publishMessage', async () => {
        ...
        expect(res.serializedContent).toBe(
            '{"type":"text","payload":"Hello World"}'
        )
    })

    it('should exercise publishMetadata', async () => {
        const res = await senderManager.publishMetadata(streamId, {
            foo: 'bar',
        })
        expect(res.serializedContent).toBe(
            '{"type":"metadata","payload":{"foo":"bar"}}'
        )
    })

    it(
        'should exercise subscribeMessages & publishMessage',
        (done) => {
    ...
    it(
        'should exercise subscribeMetadata & publishMetadata',
        (done) => {
    ...
    describe('Encryption', () => {
        ...
        it('should exercise the publishMessage method for encrypted rooms', async () => {
            const res = await senderManager.publishMessage(
                streamId,
                'Hello Encryption'
            )
        ...
        it('should receive properly decrypted messages on encrypted room', (done) => {
            ...
            senderManager
                .publishMessage(streamId, 'Hello Encryption')
                .then((sent) => {
                    // eslint-disable-next-line jest/no-conditional-expect

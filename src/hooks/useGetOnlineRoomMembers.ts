...
const TrackerRegistrySmartContractAddress = '0xab9BEb0e8B106078c953CcAB4D6bF9142BeF854d'

export default function useGetOnlineRoomMembers(): OnlineRoomMemberGetter {
    const {session: {streamrClient, wallet}} = useStore()
    ...
    return useCallback(async ({ streamId }: Options) => {
        ...
        return Object.keys(json[`${streamId}#0`]).map((key) => key.split('#')[0])

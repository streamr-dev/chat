...
export class ChatRoomManager {
    ...
    private async getStreamIdFromRoomName(roomName: string, requireEncryptedData = false): Promise<string> {
        const encryptionPrefix = requireEncryptedData ? 'encrypted/' : ''
        return (`${await this.client.getAddress()}/${this.ROOM_PREFIX}/${encryptionPrefix}${roomName}`).toLowerCase()
    }
    ...
    private async generateChatRoom(streamId: string, stream?: Stream): Promise<ChatRoom> {
        return {
            name: this.getRoomNameFromStreamId(streamId),
            stream: stream || await this.client.getStream(streamId),
            publishMessage: (message: string) => this.publishMessage(streamId, message),
            publishMetadata: (metadata: any) => this.publishMetadata(streamId, metadata),
            subscribeMessages: (callback: (message: ChatMessage) => void) => this.subscribeMessages(streamId, callback),
            subscribeMetadata: (callback: (metadata: any) => void) => this.subscribeMetadata(streamId, callback),
        } as ChatRoom
    }

    public async createRoom(roomName: string, requireEncryptedData = false): Promise<ChatRoom>{
        const streamId = await this.getStreamIdFromRoomName(roomName, requireEncryptedData)
    ...
    public async getRooms(): Promise<Array<string>> {
        ...
        const filtered = roomStreams.filter((stream: Stream) =>  stream.id.includes(this.ROOM_PREFIX))
        return filtered.map((stream: Stream) => this.getRoomNameFromStreamId(stream.id))
    }

    public async sendInvitation(streamId: string, recipient: string): Promise<void> {
    ...
    public async getInvitations(): Promise<Array<Stream>> {
        ...
        for (let i = 0; i < streams.length; i++){
            ...
            if (!stream.id.includes(address) && stream.id.includes(this.ROOM_PREFIX)){
    ...
    public async publishMessage(streamId: string, message: string): Promise<any> {
    ...
    public async publishMetadata(streamId: string, metadata: any): Promise<any> {
        return this.client.publish(streamId, {
            type: 'metadata',
            payload: metadata,
        } as ChatMessage, Date.now(), this.STREAM_PARTITION_METADATA)
    }

    public async subscribeMessages(streamId: string, callback: (message: ChatMessage) => void): Promise<void> {
    ...
    public async subscribeMetadata(streamId: string, callback: (metadata: any) => void): Promise<void> {
        this.client.subscribe({
            streamId,

...
export default function AddTokenGatedRoomModal({
    ...
    const params = {
        tokenIds: tokenIds.map((id) => Number.parseInt(id, 10)),
    ...
    const allSet =
        ...
        (!hasIds || params.tokenIds.length > 0)
        ...
        <Modal
            ...
            onBeforeAbort={(reason) => {
                if (reason === AbortReason.Backdrop) {
                    return tokenInfo.address === '' && tokenIds.length === 0 && tokenCount === ''
                }
            }}
        >
            ...
            >
                ...
                {!!hasIds && (
                    <>
                        <Label htmlFor="tokenId">Token IDs (comma separated)</Label>
                        <TextField
                            id="tokenId"
                            value={tokenIds.join(',')}
                            onChange={({ target }) => void setTokenIds(target.value.split(','))}
                        />
                    </>
                )}
                <PrimaryButton
